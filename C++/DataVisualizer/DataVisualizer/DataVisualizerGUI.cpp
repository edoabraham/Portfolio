...
#pragma endregion
// Constructor
SimulationPlotGUI::SimulationPlotGUI(QWidget *parent)
	: QMainWindow(parent) {
        
        // Load Application Configurations
        this->Configurations_.ReadConfig();
...
//         }
        ...
        // * [ View Mode ]
        InitializeViewModePreferenceWidgets();
...
void SimulationPlotGUI::UpdateDirectory() {
    ...
    // Read and Store RPK
    RPK_ = DataRPK();
    RPK_.Read(Configurations_.GetFilesDirectory().toStdString() + "RPactual.dat");
...
void SimulationPlotGUI::RefreshGraphs(int value) {
	...
	if(CurrentFileNumber_ < DataManager_.GetMaxNumOfFiles()) {
        ...
        // Update the Graphs
        ui.IonizationVsRadiusGraph_.RefreshGraph(CurrentFileNumber_);
        ui.DensityVsRadiusGraph_.RefreshGraph(CurrentFileNumber_);
        ui.TemperatureVsRadiusGraph_.RefreshGraph(CurrentFileNumber_);
        ui.WallRadiusVsTimeGraph_.RefreshGraph(CurrentFileNumber_);
        ui.ParticleDistributionPlot_.Refresh(CurrentFileNumber_);
        ui.CollisionEnergyPlot_.Refresh(CurrentFileNumber_);
...
void SimulationPlotGUI::UpdateViewMode(int index) {
    switch (index) {
        case COMPLEX_DISPLAY_MODE:
            ui.ChangeLabelDisplayMode(index);
            break;
...
void SimulationPlotGUI::StoreGraphData() {
    DataRPK currentRPK;
    int size = DataManager_.GetMaxNumOfFiles();
    
    // Reserve sizes
    ui.ParticleDistributionPlot_.Reserve(size);
    ui.CollisionEnergyPlot_.Reserve(size);
    
    for(int i = 0, j = DataManager_.GetMaxNumOfFiles(); i < j; i++) {
        currentRPK = DataRPK(RPK_, i);
        ui.IonizationVsRadiusGraph_.StorePoints(DataManager_.GetSnapshotData(i));
        ui.DensityVsRadiusGraph_.StorePoints(DataManager_.GetSnapshotData(i));
        ui.TemperatureVsRadiusGraph_.StorePoints(DataManager_.GetSnapshotData(i));
        ui.WallRadiusVsTimeGraph_.StorePoints(currentRPK);
        ui.ParticleDistributionPlot_.StorePoints(DataManager_.GetSavepointData(i));
        ui.CollisionEnergyPlot_.StorePoints(DataManager_.GetCollisionEnergyData(i));
    }
}
...
void SimulationPlotGUI::InitializeViewModePreferenceWidgets() {
    connect(ui.ViewModeComboBox, SIGNAL(currentIndexChanged(int)), this, SLOT(UpdateViewMode(int)));
}
